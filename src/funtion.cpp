...
// double brakePower =
//     0.3; //
//     刹车力度（0.4–0.8），值越小，越接近目标角度时减速越快（此处设为0.3，可能偏激进）
...
// double lastYEncoder = 0;
// void MoveDistancePID(vex::directionType dir, float targetDist, float
// Maxspeed,
...
//         double deltaY =
//             ((currentYEncoder - lastYEncoder) / 360) * (2 * M_PI * 1);
//         currentDist += deltaY;
...
//  */
// double getSmoothTargetVelocity(double current_distance, double
// total_distance,
...
//     double decel_distance = (cruise_speed * cruise_speed) / (2.0 *
//     max_decel);
