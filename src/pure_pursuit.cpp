...
// // 常量配置（根据机器人物理参数调整）
// // ========================
// const double WHEEL_RADIUS = 2.5;        // 里程计轮子半径（单位：厘米）
// const double ENCODER_TPR = 360.0;       // 编码器每转一圈的脉冲数（单位：度，V5电机默认360度/圈）
// const double TRACK_WIDTH = 18.5;        // 左右轮之间的距离（单位：厘米），用于差速转弯计算
// const double LOOKAHEAD_DISTANCE = 28.5; // Pure Pursuit 算法的前瞻距离（单位：厘米）
...
// double lastYEncoder = 0;                // 上一次Y轴编码器读数
...
// std::vector<Point> generateBezierPath(Point p0, Point p1, Point p2, Point p3, int num) {
...
//     lastYEncoder = y.position(degrees);
...
//     double deltaX = ((currentXEncoder - lastXEncoder) / ENCODER_TPR) * (2 * M_PI * WHEEL_RADIUS);
//     double deltaY = ((currentYEncoder - lastYEncoder) / ENCODER_TPR) * (2 * M_PI * WHEEL_RADIUS);

//     // 获取当前朝向（IMU，单位：弧度）
//     double currentTheta = IMU.rotation(degrees) * M_PI / 180.0;

//     // 将机器人坐标系下的位移转换到全局坐标系（旋转矩阵）
//     double globalDeltaX = deltaX * cos(currentPose.theta) - deltaY * sin(currentPose.theta);
//     double globalDeltaY = deltaX * sin(currentPose.theta) + deltaY * cos(currentPose.theta);
...
// std::vector<Point> resampleCurve(const std::vector<Point>& curve, double pointSpacing) {
...
//                 Point newPoint;
//                 newPoint.x = curve[j-1].x + ratio * (curve[j].x - curve[j-1].x);
//                 newPoint.y = curve[j-1].y + ratio * (curve[j].y - curve[j-1].y);
...
//         double dist = distances(currentPose.x, currentPose.y, path[i].x, path[i].y);
//         if (dist < minDist) {
//             minDist = dist;
//             closestIndex = i;
//         }
...
// void bezierPurePursuit(const std::vector<Point>& path, int& currentTargetIndex) {
...
//     double robotDx = dx * cos(-currentPose.theta) - dy * sin(-currentPose.theta);
//     double robotDy = dx * sin(-currentPose.theta) + dy * cos(-currentPose.theta);
...
//     double distToTarget = distances(currentPose.x, currentPose.y, targetPoint.x, targetPoint.y);
//     double baseSpeed = MIN_SPEED + (MAX_SPEED - MIN_SPEED) * std::min(1.0, distToTarget / 50.0);

//     // 差速驱动速度分配（结合曲率和朝向修正）
//     double leftSpeed  = baseSpeed * (1 + curvature * TRACK_WIDTH / 2) + headingCorrection;
//     double rightSpeed = baseSpeed * (1 - curvature * TRACK_WIDTH / 2) - headingCorrection;
...
//     // 显示当前位姿信息
//     Brain.Screen.setPenColor(white);
//     Brain.Screen.setCursor(1, 30); Brain.Screen.print("x:%.2f cm", currentPose.x);
//     Brain.Screen.setCursor(2, 30); Brain.Screen.print("y:%.2f cm", currentPose.y);
//     Brain.Screen.setCursor(3, 30); Brain.Screen.print("theta:%.2f deg", currentPose.theta * 180 / M_PI);
// }
