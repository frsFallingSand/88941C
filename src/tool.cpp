...
void IMU_Display() {
  ...
  while (true) {
    ...
    Brain.Screen.drawRectangle(graphX + 1, graphY + 1, graphWidth - 2,
                               graphHeight - 2);
    // 重新绘制中心线
    Brain.Screen.setPenColor(green);
    Brain.Screen.drawLine(graphX, zeroY, graphX + graphWidth, zeroY);
    ...
    for (int i = 0; i < maxPoints - 1; i++) {
      ...
      if (angleHistory[idx1] != 0 || angleHistory[idx2] != 0) {
        ...
        int y1 = zeroY - ((angleHistory[idx1] - TargetAngle) * graphHeight /
                          180); // 缩放因子
        int y2 =
            zeroY - ((angleHistory[idx2] - TargetAngle) * graphHeight / 180);
